...
let _refreshToken: string | null = localStorage.getItem('felix_dash_refresh_token');
...
async function refreshTokenFn() {
  ...
  if (!res.ok) { clearTokens(); window.location.href = '/login'; throw new Error('Refresh failed'); }
...
export const apiClient = createFelixClient({ baseURL: BASE_URL, getAccessToken, refreshToken: refreshTokenFn });
